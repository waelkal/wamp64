...
function btnRemove(rowNum, invoicetype,isprint=false) {
  //console.log(rowNum);
  var $$ = Dom7;
  if (isprint === true) {
    app.dialog.confirm('You Cannot delete item !')
  }
...
function invoice_parent_line(inv_id, inv_type, client_id, client_name, description, iscash,  whcode, warehid, salesmanid, locationid,locationcode,disc) {
...
function refreshTotals(inv_type, disc,counter) {
  ...
  var totalvat = 0;
  var gtotalvat = 0;
  var IsInvoiceCash = $$("#cash").prop('checked');
  var cashvalue = IsInvoiceCash ? 1 : 0;
  ...
  invoice.push(new invoice_parent_line(inv_id, inv_type, customer_id, customer_name, inv_desc, cashvalue , whcode, warehid, salesmanid, locationid, locationcode,disc));
...
async function generateJV(mainId, current_JV) {
  ...
  var invDate = new Date(parent_inv_data["inv_edate"]);
  var year = invDate.getFullYear();
  var month = invDate.getMonth() + 1; // Adding 1 because getMonth() returns 0-based month (0 for January)
  var day = invDate.getDate();
  var formattedDate = (day < 10 ? '0' + day : day) + '-' + (month < 10 ? '0' + month : month) + '-' + year ;
  $$("#Print_Date").val(formattedDate);
  ...
  for (var x in customerlist.custlist) {
    if (customerlist.custlist[x]['customerid'] == parent_inv_data["client_id"]) {
      ...
      $$("#region").val(customerlist.custlist[x]['region']);
  ...
  //col1.innerHTML = "Amount: <h3>" + total + " USD</h3>";
  starting_row++;
}
...
async function generateInvoice(mainId, current_inv) {
  ...
  var PaymentMethod = parent_inv_data["iscashvalue"];
  if (PaymentMethod == 1) {
    PaymentMethod = "CASH";
  }
  else {
  ...
  var col4 = row.insertCell(3);
  col1.innerHTML = "";
  col2.innerHTML = "<small>TotalUSD:</small>";
  col3.innerHTML = "&nbsp;&nbsp;" + total.toFixed(2) + "";
  col4.innerHTML = "";
  ...
  var col4 = row.insertCell(3);
  col1.innerHTML = "";
  col2.innerHTML = "<small>NET USD:</small>";
  ...
  var col4 = row.insertCell(3);
  col1.innerHTML = "";
  col2.innerHTML = "<small>Payment Method:</small>";
  col3.innerHTML = "&nbsp;&nbsp;" + PaymentMethod + "";
  col4.innerHTML = "";
  col4.className = 'tb_invoice_center';
  starting_row++;
...
async function printJV(mainId, current_jv) {
  ...
  $$('#JVPOS').css('display', 'block');
  printJS('JVPOS', 'html');
  $$('#JVPOS').css('display', 'none');
  // need to remove add rows in case
...
function clearItems() {
  ...
  $$("#cbocustomer").value = "nullcustomer" ;
  $$('#tblbody').empty();
  $$("#txtQTotals").val(0);
  $$("#txtGTotals").val(0);

  $$("#cbocustomer").val("nullcustomer");
  $$("#cbocustomer").change();
...
function onchangeItem(selectObject) {
  if (selectObject.selectedIndex > 0) {
    ...
    var objqty = selectObject.options[selectObject.selectedIndex].attributes['qty'].value;
    var objdesc = selectObject.options[selectObject.selectedIndex].attributes['data-description'].value;
    var objorigin = selectObject.options[selectObject.selectedIndex].attributes['data-stk_origin'].value;
    var objvat = selectObject.options[selectObject.selectedIndex].attributes['data-stk_vat'].value;
    if (objprice != null) {
...
function btnUpdate() {
  ...
  if (stkbar_id == null) {
    app.dialog.alert("Item name is null !");
      return;
  }
...
function AddCheckINOut(currentCustomer, CurrentDate, actions) {
  //create guid
  var NEW_ID = app.utils.id('xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx');
  var dialog = app.dialog.preloader('Loading..', 'blue');
  dialog.open(); 
  app.request({  
    url: 'https://unopwa.app/api/checkInOut.php',
    crossDomain: true,
    async: true, // async processing
    data: {
      'dbhost': localStorage.getItem('Server'),
    ...
    success: async function (data, status, xhr) {
      ...
      if (data == "1") {
        ...
        dialog.close();
        app.dialog.alert(actions + '. Thank you!');

      } else {
        dialog.close();
        console.log(data);
        app.dialog.alert('Failed to ' + actions);
      }
...
//function checkINOut_line(id,customerid, custsalesmanid, checkInDatetime, checkOutDatetime) {
//  this.customerid = customerid;
//  this.id = id;
//  this.custsalesmanid = custsalesmanid;
...
function UpdatecheckInOutlocaly(data, keyid) {
  // Saving data to local database first
  console.log(data);
  const dbPromise = idb.open('unodbmobile', localStorage.getItem('CurrentDBVersion'));
  dbPromise.then(db => {
    const tx = db.transaction('checkin_checkout', 'readwrite');
    tx.objectStore('checkin_checkout').put(data[0]);
    return tx.complete;
  });
}
