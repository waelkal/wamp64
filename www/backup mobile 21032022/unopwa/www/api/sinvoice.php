...
try {
    ...
    foreach($invoice as $inv) {
        if ($isFirst == true) {
            $strSQL = "Insert into inv_parent(inp_id,customer_id,warehid,inp_vdate,inp_desc,inv_type,lat,lng,salesman_id,disc,location_id,location_code) Values ('";
            $strSQL .= $inv['inp_id'] . "','" . $inv['client_id'] . "','" . $inv['warehid'] . "','" . $inv['inv_edate'];
            $strSQL .= "','" . $inv['description'] . "','" . $inv['inv_type'] . "'," . $inv['lat'] . "," . $inv['lng'] . ",'" . $inv['salesmanid'] . "','" . $inv['disc'] . "','" . $inv['locationid'] . "','" . $inv['locationcode'] . "');";
            $inp_id = $inv['inp_id'];
            $isFirst = false;
            $dblink->query($strSQL);
        } else {
